...
emp_ssn = Employee.query.with_entities(Employee.ssn).all()
empChoices = [(row[0],row[0]) for row in emp_ssn] 

prj_num = Project.query.with_entities(Project.pnumber).all()
prjChoices = [(row[0],row[0]) for row in prj_num] 

class EmployeeForm(FlaskForm):
    ssn = SelectField("Employee's SSN", choices = empChoices, validators=[DataRequired()])
    pnumber = SelectField("Project Number", choices = prjChoices, coerce = int, validators=[DataRequired()])
    ...
    def validate_pnumber(self, pnumber):
        proj = Works_On.query.filter_by(essn=self.ssn.data, pno=self.pnumber.data).first()
        if proj:
            raise ValidationError('The employee is already assigned to this project.')
